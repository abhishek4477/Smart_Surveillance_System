...
class FaceCollector:
    def __init__(self, 
        ...
        self.person_names = {}  # Store person_id to name mapping
    
    def start_collection(self, person_id: str, person_name: str):
        ...
        logger.info(f"Started collecting embeddings for {person_name} (ID: {person_id})")
    ...
    def process_frame(self, frame: np.ndarray) -> Optional[np.ndarray]:
        ...
        if results.detections:
            for detection in results.detections:
                ...
                # Add name label if collecting
                if self.current_person_id:
                    label = f"Collecting: {self.current_person_name}"
                ...
                if (self.current_person_id and 
                    ...
                    if encoding:
                        ...
                        logger.info(f"Collected embedding {len(self.collected_embeddings)}/{self.max_embeddings} for {self.current_person_name}")
...
def main():
    ...
    collector.start_collection(person_id, person_name)
            ...
            if key == ord('q'):
                ...
                if len(collector.collected_embeddings) >= collector.max_embeddings:
                    ...
                    if representative_embeddings:
                        logger.info(f"Found {len(representative_embeddings)} representative embeddings for {collector.current_person_name}")
                else:
